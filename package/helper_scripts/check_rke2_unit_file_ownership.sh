...
server_unit_file_ownership() {
    ...
    if [ "${system_type}" = "rhel" ]; then
        res=$(stat -c %U:%G "${RHEL_PATH}/${binary_name}-server.service")
        ...
        res=$(stat -c %U:%G "${OTHER_PATH}/${binary_name}-server.service")
        if [ "${res}" != "root:root" ]; then
            echo "false"
            exit
        fi
...
agent_unit_file_ownership() {
    ...
    binary_name=$2
    if [ "${system_type}" = "rhel" ]; then
        res=$(stat -c %U:%G "${RHEL_PATH}/${binary_name}-agent.service")
        if [ "${res}" != "root:root" ]; then
            echo "false"
            exit
        fi
        ...
        res=$(stat -c %U:%G "${OTHER_PATH}/${binary_name}-agent.service")
        if [ "${res}" != "root:root" ]; then
            echo "false"
            exit
        fi
