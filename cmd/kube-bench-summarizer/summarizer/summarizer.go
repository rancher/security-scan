...
func NewSummarizer(inputDir, outputDir string) *Summarizer {
	return &Summarizer{
...
func (s *Summarizer) summarizeForHost(hostname string) error {
	...
	for _, fileName := range resultFiles {
		...
		if err != nil {
			logrus.Errorf("error reading file %+v: %v", fileName, err)
			return err
		}
		...
		for _, group := range results.Groups {
			for _, check := range group.Checks {
				...
				if check.Scored && check.State != kb.PASS {
...
func (s *Summarizer) save() error {
	...
	if err != nil {
		logrus.Errorf("error writing report file: %v", err)
		return err
	}
...
func (s *Summarizer) Summarize() error {
	...
	if err != nil {
		logrus.Errorf("error listing directory: %v", err)
		return err
	}
	...
	for _, hostDir := range hostsDir {
		...
		if err := s.summarizeForHost(hostname); err != nil {
			logrus.Errorf("error summarizeForHost: %v", hostname)
		}
	}
...
func (s *Summarizer) printSummaryReport() error {
	...
	if err != nil {
		logrus.Errorf("error marshalling summary report: %v", err)
		return err
	}

	txt := string(bytes)
	logrus.Infof("txt: %+v", txt)
	return nil
}
